...
# Check if the working directory ends with "CLIF 1.0" or "vent_variation"
if (grepl("CLIF-1.0$", current_dir)) {
  path_clif_vent_variation <- here("projects", "vent_variation")
  
  # if you are doing things in the vent_variation folder you can just stay there
} else if (grepl("vent_variation$", current_dir)) {
  path_clif_vent_variation <- ""
  
  # if you are doing things in a folder outside of git_hub, we will just make this your path
} else {
  path_clif_vent_variation <- ""  
}

# Print the result
print(path_clif_vent_variation)
...
path_components <- unlist(strsplit(current_path, "/"))

# Determine how many levels to go up (2 in your case)
levels_up <- 2

# Remove the last 'levels_up' components from the path
parent_path <- paste(path_components[1:(length(path_components) - levels_up)], collapse = "/")

# Add the sibling folder name
sibling_folder_path <- file.path(parent_path, "CLIF-1.0", "mCIDE")

# Define the file name within the sibling folder
file_name <- "clif_vocab_labs.csv"

# Access the target file
target_file_path <- file.path(sibling_folder_path, file_name)

# Now read the file
read_csv(target_file_path)
...
## File type
 ...
 ## CLif 1.0 path
  path_clif_1.0           = "Y:/DataStageData/Nick/CLIF/CLIF-1.0",
 
 ## Project Path
  # Where is your current directory for this project that  you want to work in
  path_vent_variation     = "Y:/DataStageData/Nick/CLIF/CLIF-1.0/projects/vent_variation/",
...
# Save as Parquet
write_parquet(site_specific_info, "site_specific_information.parquet")
...
site_specific_info <- read_parquet("site_specific_information.parquet")
